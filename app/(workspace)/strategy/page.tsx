  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
      <Card className="rounded-2xl border-zinc-200 shadow-sm">
        <CardHeader>
          <CardTitle className="text-base">Market input</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="co">Company name</Label>
            <Input id="co" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Northline Digital" className="rounded-xl" />
          </div>
          <div className="space-y-2">
          ...
          <div className="rounded-2xl border border-zinc-100 bg-zinc-50/80 p-4 text-sm text-zinc-600">
            Produces a directional plan and competitor landscape using consistent market assumptions.
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            ...
            {!loading &&
              (workspace?.competitors.length ? (
                ...
                <p className="text-sm text-zinc-500 md:col-span-2">Run strategy to load competitor cards.</p>
              ))}
          </CardContent>
        </Card>
        <Card className="rounded-2xl border-zinc-200 shadow-sm">
          ...
          <CardContent className="space-y-3 text-sm text-zinc-700">
            ...
            {!loading && !workspace?.strategy && <p className="text-zinc-500">Generate strategy to populate this section.</p>}
          </CardContent>
        </Card>
      </div>
    </div>
